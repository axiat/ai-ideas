...
- 解释公认现象:对失败、成功、scaling 曲线或涌现行为给出竞争性解释并设计判别实验
- 换数据来源:仿真/人类视频/互联网视频/跨本体数据
- 换时间尺度:长时程规划、亚秒级反应控制、记忆与上下文长度
- 换评测对象:评数据、评环境、评指标本身,或指认被忽略的混杂变量并量化其影响
- 换规模轴:极小模型/边缘部署/预算砍一个量级,或把深度/上下文/视角数/时程推高一个量级
- 换学习信号:无奖励、自监督、物理先验、几何一致性
- 换输出表征:latent action/轨迹/光流/affordance/子目标替代直接动作预测
- 统一或拆分:把两个一直分开处理的问题并成一个机制,或把一个耦合系统拆开
- 闭环与经验:部署数据回流/RL post-training/人在环干预/test-time 适应
