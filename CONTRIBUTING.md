...
Documentation changes must keep relative links valid. Tracked product text may contain only sparse Han, such as historical artifact tokens; dense Han still fails the product contract. Evidence archives under [`calib/observations/`](calib/observations/README.md) retain their original language and bytes, including frozen source whitespace, to preserve recorded hashes. The current 804-row canonical ledger export also retains its source language; this exception requires the exact raw-file SHA-256 stored as `EXPECTED["ledger_snapshot"]` in `tests/verify_product_contract.py`. A baseline update requires verification against canonical history and decision archives, preserving all existing rows. These exceptions do not apply to active calibration fixtures, runtime prompts, or product documentation. The product hero path is `assets/ai-ideas-hero.png`.
