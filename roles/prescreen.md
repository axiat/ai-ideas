...
- **命题式 idea**(头条是一句关于世界的断言、非 M×D 配对——竞争解释/命名新问题/指标测错了量/删承重假设)的"头条发现"也包括:该命题被某单篇正面回答,或被 idea 点名的靶子论文其摘要/limitation 自认。后者只需打开那一篇靶子核对、不额外铺检索(竞争解释/estimand 是否已命名等的系统检索留给深查),保持预筛便宜。
...
- 判定行整行只能是 `判定:keep` 或 `判定:kill`,不得附加任何词;附加词视为判定非法,orchestrator 按 keep 兜底(kill 白判)。
