...
- `tmp/near-sa-queue.tsv`(若存在;near-SA 修订队列——拿到过 SA 票、被判 accept-w-rev + overlap=low 的候选,进化/复查名额**优先**取它,见下方"进化")
