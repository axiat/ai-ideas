#!/usr/bin/env bash
# AwR 复活 sidecar:主环之外用多轮 agy(便宜可错)把 ledger 中 accept-w-rev 的 idea 改进成
# 可复审的成品。研究员 agy 检索补缺口、出修订稿(roles/awr.md);裁判 agy 按主环 rubric 判
# 「SA-可能/还不行」(roles/awr-judge.md);判还不行则缺陷回灌任务文件,研究员下轮继续改;
# MAX_ROUNDS 轮反馈用尽则带最后修订稿收尾。成品 tmp/agy-side/awr/<key>.md 给人/claude 复审,
# verdict 不进主环。只读 ledger 基线、roles/awr*.md、rubric.md、brainstorming_policy.md;
# 只写 tmp/agy-side/awr/;不碰 tmp/round/、ideas/、不 git。与 hunt.sh 可同时跑。
#
# 对 agy 三个已知弱点的对策:
...
#
# 用法: caffeinate -is ./agy-side.sh        # 常驻:队列全终态后每 POLL 秒重扫(等主环产新 AwR)
...
while ! mkdir "$sidelock" 2>/dev/null; do
  ...
  echo "agy-side: 已有实例在跑(pid ${holder:-未知}),退出。确认无实例可 rm -rf $sidelock" >&2; exit 1
done
echo $$ > "$sidelock/pid"
trap 'rm -rf "$sidelock"; [ "$(cat "$gate_lock/pid" 2>/dev/null)" = "$$" ] && rm -rf "$gate_lock"' EXIT
rm -rf "$repo"/tmp/agy-side/run.* 2>/dev/null || true   # 上次被 kill 遗留的调用镜像;已持实例锁,无并发在用
...
run_agy() {
  gate
  local target=$1 prompt=$2 logf=$3 sandbox rel target_in_sandbox prompt_in_sandbox pre rc
  sandbox=$(mktemp -d "$repo/tmp/agy-side/run.XXXXXX") || return 1
  rel=${target#"$repo"/}
  target_in_sandbox="$sandbox/$rel"
  mkdir -p "$sandbox/roles" "$sandbox/tmp/agy-side/awr" "$(dirname "$target_in_sandbox")"
  ...
  cp "$outdir"/*.md "$sandbox/tmp/agy-side/awr/" 2>/dev/null || true
  rm -f "$target" "$target_in_sandbox"
  prompt_in_sandbox=${prompt//$repo/$sandbox}
  pre="仓库根(绝对路径)= ${sandbox}。当前工作目录已在此根下,所有读写路径一律相对该根解析。本次任务只允许写一个文件:${target_in_sandbox}。真实仓库未作为 agy 工作目录提供;严禁写 tmp/round/、ideas/、ledger.tsv,严禁写 ~/.gemini、任何 scratch 目录或 \$HOME 其它位置。"
...
while :; do
  ...
  while IFS=$'\t' read -r d source theme idea verdict reason overlap <&3; do
    ...
    if ! { [ -s "$draft" ] && [ "$draft" -nt "$task" ]; }; then
      ...
      run_agy "$new" "读 ${repo}/roles/awr.md,按其执行;任务输入在 ${task}${hint};产物写 ${new}。" "$alog"; rc=$?
    ...
    run_agy "$judgef" "读 ${repo}/roles/awr-judge.md,按其执行;待评草稿在 ${draft},任务背景在 ${task},评分标准在 ${repo}/rubric.md 与 ${repo}/brainstorming_policy.md;判定写 ${judgef}(覆盖旧内容)。" "$alog"; rc=$?
