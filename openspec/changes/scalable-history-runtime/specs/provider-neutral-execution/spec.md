...
#### Scenario: Portable version schema avoids the Agy numeric-enum dialect
- **WHEN** the host builds any portable stage response schema
- **THEN** its `schema_version` property is exactly `{"maximum":1,"minimum":1,"type":"integer"}`, and provider request, frozen bytes, inline argv, schema echo, and inner validation use that same schema
- **AND WHEN** either bound is missing, extra `enum` or `const` appears, the bounds differ, either bound is not the built-in integer `1`, or the inner value is not the built-in integer `1`
- **THEN** the host rejects the schema or value before import, projection, and completion
- **AND WHEN** agy rejects the inline schema before prompt or model work
- **THEN** the host retains only preflight state, removes the disposable attempt, and creates no import, projected output, or completion receipt
- **AND THEN** the legacy history-stage adapter retains its original response schema because normalization occurs only on the portable copy
