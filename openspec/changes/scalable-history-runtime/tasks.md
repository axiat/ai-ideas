...
- [ ] 8.4 Qualify the binding-covered Grok final-response fence and reducer-joined unique-terminal-fence transport with a live Grok `awr-judge` smoke; the offline transport and final-round no-sleep regressions are implemented.
- [ ] 8.5 Qualify binding-covered non-Grok raw canonical stdout instructions with a live agy `awr-judge` smoke; offline request-binding, all-agy AwR, closed-path/type/link/mode/content-integrity, canonical-schema, and attestation regressions are implemented.
