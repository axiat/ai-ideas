...
def verify_common(manifest):
    ...
    if digest!=cli['config_sha256']:
        # Codex adds trusted entries for the exact disposable mirrors it starts.
        # Remove only those complete, strictly shaped blocks; every other byte
        # must still match the original run configuration.
        if raw is None:raise ValueError('Codex config disappeared; no provider launched')
        blocks=list(re.finditer(rb'(?m)^\[projects\.[^\n]+\]\n(?:(?!\[)[^\n]*\n?)*',raw))
        normalized=raw
        mirror_pattern=re.compile(re.escape(str(ROOT))+r'/run/(A|B)/probe[0-9]+/seat-[123]/state/attempt-[^/]+/mirror')
        for block in reversed(blocks):
            parsed=tomllib.loads(block.group().decode('utf-8'))
            entries=parsed.get('projects',{})
            if len(entries)!=1:continue
            name,settings=next(iter(entries.items()))
            if mirror_pattern.fullmatch(name) and settings=={'trust_level':'trusted'}:
                normalized=normalized[:block.start()]+normalized[block.end():]
        desktop_blocks=list(re.finditer(rb'(?m)^\[desktop\]\n(?:(?!\[)[^\n]*\n?)*',normalized))
        for block in reversed(desktop_blocks):
            desktop=tomllib.loads(block.group().decode('utf-8')).get('desktop',{})
        ...
        if hashlib.sha256(normalized).hexdigest()!=cli['config_sha256']:
            raise ValueError('Codex config changed beyond proven inert trust/desktop metadata; no provider launched')
