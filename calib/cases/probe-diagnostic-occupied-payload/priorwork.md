...
受控 payload occupancy 事实（synthetic，仅供 policy regression）:同一检索边界内的最近实践已经实现完全相同的 sensitivity-conditioned acquisition rule——每任务 2 条 pilot 后只给 action-sensitive strata 增补 3 条，所有选择与调参见过的 target-task labels 均计入 5 条上限——并在相同 AMPLIFY architecture、LIBERO 五 suite 与 success-rate metric 上，于预注册 -0.05 容差内匹配 AMPLIFY-50，实现同一 10× action-label reduction payoff；此受控事实不改变 headline `overlap=low`。
