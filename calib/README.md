...
- `probe-diagnostic-occupied-payload` / `probe-diagnostic-incremental-payload` — 配对 synthetic policy regression，只改变“同一 2+3 sensitivity-conditioned acquisition 与 10× matched-performance payoff 是否已占”这一条受控事实，检查 verdict policy 是否同时封住重复记账、保留 clear 净增量出口。occupancy 事实是 synthetic；两案 `expect` 均为 `probe`，结果不得进入正式 calibration accuracy，也不得声称为论文或外部 ground truth。
