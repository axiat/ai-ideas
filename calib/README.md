...
- `neg-meanflow-mp1` — MeanFlow→动作生成迁移故事,被 MP1(2507.10543,2025-07,带代码)在 ICLR 2026 投稿前 72 天直接占据,且"一步化全靠蒸馏"前提早被 FlowPolicy(2412.04987,AAAI 2025 oral)/AdaFlow(2402.04292)证伪。原为 pos-meanflow(Mean Flow Policy,ICLR 2026 oral 2602.13810):核实发现真实 oral 靠 IVC 增量+RL 赛道错位+全文未引 MP1/FlowPolicy 存活,不构成"诚实全知查重下仍值 SA"的阳性,改作 direct-hit 阴性(2025 新占位,补 neg-replai 的 2022 老占位形态)。method 型 **SA** 阳性席位仍空缺(pos-muninn/pos-lowutd 为 method 型但 ground truth 是 AwR),选定标准:cutoff 后 oral/spotlight/award 且**头条机制经诚实查重仍零占据**(payoff 落在近期已占轴上时 idea 形态几乎必然封顶 AwR,见 `results-2026-07-20.md`)。
...
- `pos-muninn` — **method 型 AwR 正对照**:RSS 2026 Outstanding Student Paper「Muninn」(arXiv 2605.09999,post-cutoff)的投稿前形态,纯新机制。头条(缓存决策的分布无关误差保证)真零占据,但加速 payoff 已被 BAC(2506.13456)/Sparse ActionGen(2601.12894)占据,净增量只剩证书本身。ground truth = AwR(expect: min_vote≥AwR):测面板不为「已占轴上加证书」超付 SA。3/3 AwR(见 `results-2026-07-20.md`)。
- `pos-lowutd` — **method 型 AwR 正对照**:RSS 2026 Outstanding Paper「FlashSAC」(arXiv 2604.04539,post-cutoff)的投稿前形态,纯新机制(sub-1 极端 UTD × 规模化 × 机器人 sim-to-real)。机制轴 CrossQ(UTD=1)先占、payoff 轴 FastTD3/FastSAC 占据,净增量是重组+一条梯度范数。ground truth = AwR。**命名铁律**:case 名不得含原型论文名——首轮 `pos-flashsac` 被裁判拿目录名猜身份,改名后重跑。3/3 AwR(见 `results-2026-07-20.md`)。
