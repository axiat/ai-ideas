...
1. **生成**(`roles/generate.md`):读 policy、ledger 与失败清单,先发散 10 个候选,再自筛出 4-6 个差异最大的 idea 到 `tmp/round/`,遵守 policy 发散要求、主题反坍缩与四种合法形态,避开 ledger 已有行、死因模式和已饱和套路;每个 idea 标注主题并附最小否证实验(点名最强基线、给样本量与预期效应);发散透镜由 `hunt.sh` 随机抽取注入。
