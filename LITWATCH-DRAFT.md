...
主循环的查重本来就用 WebFetch 打 arXiv API(`http://export.arxiv.org/api/query?search_query=...`)和 Semantic Scholar(`https://api.semanticscholar.org/graph/v1/paper/search?query=...`)。litwatch 取数(`lib/litwatch.py`,python 标准库 urllib,零依赖)打这两个**同样的端点**,按主题拉近 N 天近作。
...
- **S2 免费额度**:确认无 key 基本 429。已默认关 s2、只跑 arxiv;要用 s2 设 `LITWATCH_S2_KEY`(x-api-key 头)。
- **网络错误**:429 / 超时 / 连接错原会 traceback,已改成干净跳过、返回空、pipeline 续跑(真机复现并验过)。
- **agy 标注路径成立**:真机 dry-run(经 agy-worker.sh,~46s,无登录卡)——agy 正确读沙箱 staging + `roles/litwatch.md`,按主题研判,遵守「拿不准不标」,产出干净空 `annotations.jsonl`;沙箱隔离与零回归端到端成立。agy 能安全干这活。
- **未决:取数相关性(决定整个 cache 有没有价值)**。arXiv `all:` 空格分词 + date 排序是松散 OR,召回近作但离题(真机拿到并行编程 / 膳食评估 / 跌倒检测),agy 因此正确地一条没标 = cache 零价值。已把排序做成 `LITWATCH_SORT`(relevance 可选)、主题支持 arXiv 布尔表达式,但 arXiv 对本 IP 今日多次突发已限流(空响应 / 超时),relevance / 布尔主题的实际相关性没能当场验掉。**需一次未被限流的运行(换时段 / 换 IP,如集群 xyh)肉眼核 cache 是否真出有用近邻——在此之前 cache 价值未证。**
