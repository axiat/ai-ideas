...
2. **push 守卫**:`.githooks/pre-push` 拒绝直推 main,agent 产出一律走特性分支 + PR,合入只经 CI 守卫。`hunt.sh` 启动时自动 `git config core.hooksPath .githooks`;新 clone 若只跑单次会话,先手动执行一次该命令。人工直推:`ALLOW_MAIN_PUSH=1 git push`。绕过面:`git push --no-verify` 跳过本 hook,且 GitHub 端未启用 branch protection,远端 main 无服务端防线。
